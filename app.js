...
function refresh() {
  const d = PAIRS[cp];
  const j = cp === 'XAUUSD' ? (Math.random() - 0.5) * 4 : (Math.random() - 0.5) * 0.0012;
  d.price = parseFloat((d.price + j).toFixed(cp === 'XAUUSD' ? 2 : (cp === 'USDJPY' || cp === 'GBPJPY' ? 3 : 5)));
  render();
}
...
function renderMetrics(d) {
  ...
  const wLabels = {A:'Fase A',B:'Fase B',C:'Fase C',D:'Fase D',E:'Fase E'};
  document.getElementById('metricsGrid').innerHTML = `
    ...
    <div class="metric-card">
      ...
      <div class="metric-val mv-white" style="font-size:14px">${wLabels[d.wyckoffPhase]}</div>
...
function drawChart(d) {
  ...
  const bg = '#111111', gc = 'rgba(255,255,255,0.04)', tc = '#4a4845';
  ...
  for (let i = 0; i <= 5; i++) {
    ...
    ctx.fillStyle = tc; ctx.font = '10px "Space Mono", monospace'; ctx.textAlign = 'right';
    ctx.fillText(fp(pv), pad.l - 5, y + 3);
  }

  const lines = [
    [d.vah, '#a78bfa', 1, [3,3], 'VAH'],
    [d.val, '#60a5fa', 1, [3,3], 'VAL'],
    ...d.supports.map(s => [s, '#3de89a', 1, [4,3], 'S']),
    ...d.resistances.map(r => [r, '#ff5c3e', 1, [4,3], 'R']),
    [d.ema200, '#7f77dd', 1.5, [], 'EMA200'],
    [d.ema50, '#378add', 1.5, [], 'EMA50'],
    [d.ema20, '#ef9f27', 1.5, [], 'EMA20'],
    [d.poc, '#f5c842', 2, [], 'POC'],
  ];
  ...
  ctx.fillRect(W - pad.r - 68, py - 10, 66, 18);
...
function renderVolumeProfile(d) {
  ...
  const rows = d.vpLevels.map(l => {
    ...
    const tag = isPoc ? '<span class="vp-tag tag-poc">POC</span>' :
                isVAH ? '<span class="vp-tag tag-vah">VAH</span>' :
                isVAL ? '<span class="vp-tag tag-val">VAL</span>' :
                l.cur ? '<span class="vp-tag tag-cur">←</span>' : '';
    return `<div class="vp-row">
      <span class="vp-price">${fp(l.p)}</span>
      <div class="vp-bar-outer"><div class="vp-bar-inner" style="width:${w}%;background:${col}"></div></div>
      ${tag}
    </div>`;
  }).join('');
  ...
  document.getElementById('vpContent').innerHTML = `
    ...
    <div style="margin-top:10px;padding:8px 12px;background:#1a1a1a;border-radius:4px;font-size:11px;color:#8a8780;line-height:1.6;border-left:2px solid #f5c842">
      Preço ${d.price > d.poc ? 'acima' : 'abaixo'} do POC — viés ${d.price > d.poc ? 'comprador' : 'vendedor'}.
...
function renderHierarchy(d) {
  ...
  const confDots = Array(8).fill(0).map((_, i) => {
    ...
    return `<div class="conf-dot ${c}" title="${i+1}/8"></div>`;
  }).join('');
  ...
  document.getElementById('hierarchyContent').innerHTML = hier.map((h, i) => `
    <div class="hier-step">
      ...
      <div class="hier-body">
        <div class="hier-name" style="color:${h.color}">${h.name}</div>
        <div class="hier-desc">${h.desc}</div>
      </div>
    </div>`).join('') +
    `<div style="margin-top:10px;padding:8px 12px;background:#1a1a1a;border-radius:4px;font-size:11px;color:#8a8780;line-height:1.5">
...
function renderElliott(d) {
  const bars = d.elliottHeights.map((h, i) => {
    ...
    return `<div class="wave-bar-wrap">
      <div class="wave-lbl" style="color:${isActive?col:'#4a4845'}">${d.elliottLabels[i]}</div>
      <div class="wave-bar" style="height:${h}px;background:${isActive?col:col+'30'}"></div>
      <div class="wave-cur">${isActive?'●':''}</div>
    </div>`;
  }).join('');

  const badge = `Onda ${d.elliottWave} · ${d.elliottImpulse?'Impulso':'Corretivo'}`;
  document.getElementById('elliottBadge').textContent = badge;
  document.getElementById('elliottBadge').className = 'card-badge ' + (d.bias==='bull'?'badge-bull':d.bias==='bear'?'badge-bear':'badge-neut');

  document.getElementById('elliottContent').innerHTML = `
    <div class="wave-chart">${bars}</div>
    <div class="wave-desc">${d.elliottDesc}</div>
    <div class="wave-next"><strong>Próximo movimento:</strong> ${d.correctiveDesc}</div>`;
}
...
function renderEntry(d) {
  ...
  if (e.dir === 'AGUARDAR') {
    ...
    document.getElementById('entryContent').innerHTML =
      `<div class="entry-await">◆ Sem setup válido.<br>Aguardar rompimento com volume<br>ou conclusão da onda Elliott.</div>`;
  ...
  document.getElementById('entryContent').innerHTML = `
    <div class="entry-dir-row" style="background:${bg}">
      <div>
        <div class="entry-dir-label" style="color:${dc}">${e.dir}</div>
        <div class="entry-zone">Zona: ${e.zone}</div>
      </div>
      <div class="rr-box">
        <div class="rr-val" style="color:${dc}">1:${e.rr.toFixed(1)}</div>
        <div class="rr-lbl">risco/retorno</div>
      </div>
    </div>
    <div class="rr-bar"><div class="rr-fill" style="width:${rrW}%;background:${dc}"></div></div>
    <div style="display:flex;justify-content:space-between;font-size:10px;color:#4a4845;margin-bottom:12px;font-family:'Space Mono',monospace">
      <span>1:0</span><span>mín 1:2</span><span>1:4</span>
    </div>
    <div class="sl-row">
      <span class="sl-label">Stop Loss</span>
      <span class="sl-val">${fp(e.sl)}<span class="sl-pip">${d.pips_sl}p</span></span>
    </div>
    ${[[e.tp1,d.pips_tp1,'50%'],[e.tp2,d.pips_tp2,'30%'],[e.tp3,d.pips_tp3,'20%']].map(([tp,pip,pct],i)=>`
    <div class="tp-row">
      <span class="tp-label">TP${i+1} — fechar ${pct}</span>
      <span class="tp-val">${fp(tp)}<span class="tp-pip">+${pip}p</span></span>
    </div>`).join('')}
    <div class="entry-rule">
      <strong>Regra de ouro:</strong> Entre apenas no reteste da zona com candle de confirmação (engolfo ou pin bar). Mova SL para breakeven ao atingir TP1. Máximo 1–2% do capital por trade.
    </div>`;
}

/* ── Zones ── */
function renderZones(d) {
  const typeColors = { res: '#ff5c3e', fib: '#60a5fa', sup: '#3de89a', cur: '#8a8780' };
  const typeLabel = { res: 'Resistência', fib: 'Fibonacci', sup: 'Suporte', cur: 'Preço atual' };

  const items = d.fibs.map((f, i) => {
    const t = d.fibTypes[i] || 'fib';
    const isCur = t === 'cur';
    return `<div class="zone-item${isCur?' is-price':''}">
      <div class="zone-type zt-${t}">${typeLabel[t]}</div>
      <div class="zone-name">${d.fibLabels[i]}</div>
      <div class="zone-price" style="color:${typeColors[t]}">${fp(f)}</div>
    </div>`;
  });

  items.push(`<div class="zone-item">
    <div class="zone-type zt-poc">Volume POC</div>
    <div class="zone-name">Point of Control</div>
    <div class="zone-price" style="color:#f5c842">${fp(d.poc)}</div>
  </div>`);
